...
- **All changes land via PR; `main` is branch-protected.** Direct pushes to `main` are rejected by GitHub. Open a feature branch, push, open a PR, let CI's 6 required checks go green, then squash-merge. Squash is the only merge strategy enabled, so **the PR title becomes the squashed commit message on `main`** — write it as the conventional-commit you want to land (e.g., `feat(schemas): zod schemas for translation domain`, not `wip` or `Phase 2.1`). The PR description becomes the commit body, so put the _why_ there.
- **PR titles describe what changed (and optionally how), not the planning process or who drove it.** Write the title from the perspective of someone reading `git log` on `main` six months from now: they want to know "what does this commit do?", not "what tracker was this driven from?" or "when was the planning meeting?". Banned in titles: dates (`2026-05-03`), internal tracker names (`audit`, `review`), planning-process framing (`absorb findings`, `PR 1 of 3`), people / roles (`founder-driven`). Same rule for the PR description body — it explains the _why_ of the **change**, not the _why_ of the **planning step**. The body is the squashed-commit body on `main`; the same six-months-later test applies.
- One commit per logical change. Conventional-commit prefixes: `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `perf`, `build`, `ci`. (`build:` for build-system + dep changes per Conventional Commits spec; `ci:` for CI/workflow changes. Dependabot uses both.)
...
- Phase commits (Phase 1 → 8 in the implementation plan) are larger and follow the per-phase deliverable list. Even within a phase, prefer multiple small PRs over one giant one — each PR is one squashed commit on `main`, and small focused commits make `git log` and `git bisect` more useful.
...
- **Branch naming: `<github-user>/<what>`.** Light-touch convention: the GitHub username (or its short form, e.g. `joao` for `JoaoPucci`) of the human who owns the branch, a slash, and a kebab-case description of what it does. Examples: `joao/phase-2-schemas`, `joao/dependabot-config`, `joao/fix-ci-pnpm-version`. **Never use the AI tool's name as the prefix** (`claude/...`, `cursor/...`, `copilot/...`) — multiple humans on the same repo using the same AI would collide on the same namespace, and the branch namespace is a _who_-namespace where the assistant is a tool the human used, not the owner. Bot-generated branches (Dependabot's `dependabot/...` prefix, automated release branches) are exempt because bots own their own well-known namespaces. The point is "I can scan `git branch -r` and tell at a glance which human is doing what" — not perfect compliance.
- **Every PR has an assignee + at least one type label + relevant area labels.**
  - **Assignee:** the GitHub account that owns the work — self-assign if you opened it. If AI opened the PR on someone's behalf, AI sets the assignee to that GitHub user.
  - **Labels:** exactly one `type/*` matching the conventional-commit prefix (`type/feat`, `type/fix`, `type/chore`, `type/docs`, `type/refactor`, `type/test`, `type/perf`, `type/build`, `type/ci`), plus one or more `area/*` for the parts of the system touched (`area/web`, `area/server`, `area/db`, `area/ci`, `area/docs`, `area/security`, `area/llm`, `area/auth`). Bot PRs (Dependabot) self-label — leave them alone.
  - **Missing label?** Create it. Use namespaced naming (`type/X`, `area/X`) and pick a colour consistent with the existing palette: solid colours for `type/*`, pastels for `area/*`. Reuse the taxonomy — don't reinvent.
- **DCO sign-off required on every commit.** Each commit message ends with `Signed-off-by: Name <email>` (the standard Developer Certificate of Origin attestation per developercertificate.org). Add it via `git commit -s` — git fills in the trailer from your `user.name` / `user.email` config. Reason: the project is AGPL-3.0 and accepts external contributions; DCO is the lightweight, in-line equivalent of a CLA that records each contributor's right to submit the code under the project's license, without the friction of a separate signature ceremony. Bot commits (Dependabot, Codex auto-fixes) sign automatically. The rule is currently a **manual + reviewer expectation**; a CI DCO workflow (e.g. the `dcoapp/app` GitHub App or a `Signed-off-by`-grep job in `ci.yml`) is on the pre-launch checklist (`compliance.md §11`) — until that lands, reviewers verify on PR open.
