...
- **Lookup:** select by `(identifier, hashed_value)` together — never by `identifier` alone. Up to 5 outstanding tokens may exist per email at any moment (see Rate limits below), so a `WHERE identifier = $email` returns one row non-deterministically and rejects valid tokens whose row happens to sort second. Compute `h = sha256(submittedToken).hex()` first, then `SELECT WHERE identifier = $email AND value = $h AND expires_at > now()`. Constant-time comparison via the `timingSafeEqualBytes` wrapper from §13.6 (length-safe `node:crypto` `timingSafeEqual`) is applied to `value` as defense-in-depth even though the WHERE already narrowed by hash. DELETE only the matched row (single-use) — leaving sibling unconsumed tokens alive until they expire.
