...
- **Lookup + consume in one atomic statement:** compute `h = sha256(submittedToken).hex()`, then `DELETE FROM auth_verification_tokens WHERE identifier = $email AND value = $h AND expires_at > now() RETURNING id`. Empty `RETURNING` → reject (token already consumed, never existed, or expired). Non-empty `RETURNING` → this request is the unique consumer; establish the session. Two reasons for this exact shape: (1) `(identifier, value)` together — never `identifier` alone — because up to 5 outstanding tokens may exist per email at any moment (Rate limits below), and a `WHERE identifier = $email` returns one row non-deterministically and rejects valid tokens whose row sorts second; (2) `DELETE … RETURNING` in one statement closes the SELECT-then-DELETE TOCTOU race that would otherwise let two concurrent verifies of the same magic link both succeed, defeating the single-use guarantee. Constant-time comparison via the `timingSafeEqualBytes` wrapper from §13.6 stays on the matched row's `value` as defense-in-depth before session creation.
