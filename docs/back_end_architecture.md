...
The Drizzle `db.forUser(user)` wrapper computes `proof = user_id || ':' || hmac(server_secret, user_id)` and `SET LOCAL nuansu.session_proof = ...` at the start of every transaction. The same `server_secret` is read from env (`NUANSU_DB_SESSION_PROOF_SECRET`) on app boot and persisted into a server-only `nuansu.config(key='session_proof_secret', value bytea)` table during migration so `nuansu.verify_hmac()` (running as `nuansu_migrate` via SECURITY DEFINER) can recompute and compare. Only `nuansu_migrate` has SELECT on `nuansu.config`; the app + auth roles cannot read the secret directly even if they bypass the wrapper. Server-secret rotation is documented in `security.md §11`.

The migration pipeline runs in three phases under the `nuansu_migrate` role: (1) `apps/web/server/db/bootstrap.sql` applies the static infrastructure (extensions, the `nuansu` schema, the SECURITY DEFINER functions including the `nuansu_auth_user_to_app_user` trigger function); (2) Drizzle's generated migrations create the application + auth\_\* tables; (3) `apps/web/server/db/rls.sql` re-assigns table ownership to `nuansu_migrate`, applies role-level GRANTs to `nuansu_app` and `nuansu_auth`, attaches the `auth_user_to_app_user` AFTER INSERT trigger, and creates every RLS policy. Roles themselves (`nuansu_app`, `nuansu_auth`, `nuansu_migrate`) are **created on first bootstrap only** by `migrate.ts` (passwords parsed from `DATABASE_URL` / `AUTH_DATABASE_URL` / `MIGRATE_DATABASE_URL`). Subsequent runs do **not** ALTER existing roles — Postgres 16+ requires `ADMIN OPTION` on the target role for ALTER, which a role does not hold on itself by default, so per-run ALTER is fragile when `MIGRATE_DATABASE_URL` points at `nuansu_migrate` rather than the platform superuser. Password rotation is therefore an out-of-band operator task (a one-shot `psql` or cloud-console action), not part of every deploy. The first bootstrap runs as the platform superuser; afterwards `MIGRATE_DATABASE_URL` may point at `nuansu_migrate` provided that role is granted the privileges Drizzle's migrator needs (`CREATE` on database, ownership of the `drizzle` schema). Wiring up the non-superuser migrate workflow end-to-end is deferred to v2; v1 production runs migrations as the platform superuser.
