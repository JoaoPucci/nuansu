...
**DoD R1–R4:** A new user can complete signup → onboarding → land on an empty chat list in under 60 seconds; can log out and back in without losing any preferences; can request deletion from settings and receive confirmation email.
...
**DoD:** User can set "Aiko: Kansai-ben, naturalness 70, no emoji, she calls me Lu" once and every subsequent outbound carries that context without re-entry.
